...
export default function LeaderboardPage() {
  ...
  async function fetchData() {
      ...
      for (const e of myEntries) {
        ...
        if (bet) {
          if (e.side === bet.winner_side) {
            vunna++;
            // Net gain: loser pot * (my stake / total winner stake)
            ...
            if (totalWinnerStake > 0) {
              klunkarVunnet += Math.round((e.klunkar / totalWinnerStake) * totalLoserStake);
            }
          } else {
            forlorade++;
    ...
    // Only show users who have participated in at least one bet
  ...
  useEffect(() => {
    fetchData();
    const ch = supabase.channel("leaderboard_bets")
      .on("postgres_changes", { event: "*", schema: "public", table: "bets" }, fetchData)
      .on("postgres_changes", { event: "*", schema: "public", table: "bet_entries" }, fetchData)
      .subscribe();
